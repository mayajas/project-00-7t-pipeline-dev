...
OUTDIR=$PROJECT_DIR/manualcorr/coreg/_subject_id_${SUBJECT_ID}/${WHICHCOREG}_test
...
if [ ! -f "$OUTDIR/corrected_meanFunc.nii" ]; then
...
if [ "$WHICHCOREG" = "struct2func" ]; then
    ...
    MANUAL_MASK=$OUTDIR/fixedImageMask_T1.nii
    antsRegistration --dimensionality 3 --float 1 \
            ...
            --convergence [ 1000x500x250, 1e-06, 10 ] --smoothing-sigmas 2.0x1.0x0.0 --shrink-factors 4x2x1 --use-histogram-matching 0 \
    ...
    antsApplyTransforms --default-value 0 \
        ...
        --transform $OUTDIR/regT12FUNC_Composite.h5
else
