...
sRF_size = np.empty((1,len(eccs),)).tolist()
sRF_size = list(itertools.chain(*sRF_size))
...
depth = np.tile(np.arange(0.2, 1.0, 0.1),8)
...
sRF_size = np.empty((1,len(depth),)).tolist()
sRF_size = list(itertools.chain(*sRF_size))
...
for sub_id in range(0,len(subject_list)):
    ...
    for hem_id in range(0,len(hem_list)):
        ...
        # data directories
        prfpy_dir    = '/home/mayajas/scratch/project-00-7t-pipeline-dev/output/prfpy_DoG_Iso2DGaussianModel/'+subject_list[sub_id]
        ...
        # prfpy output files
        grid_fit_fn      = opj(prfpy_dir,hem_list[hem_id]+'_grid_fit_dog.pckl')
        iterative_fit_fn = opj(prfpy_dir,hem_list[hem_id]+'_iterative_fit_dog.pckl')
        ...
        for layer in range(2,np.size(np.unique(masked_lay_equivol))):
            ...
            p_val=np.nanmean(prf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            s_val=np.nanmean(srf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            #print("Ecc 2: "+str(p_val))
            prf_size_ecc_2.append(p_val)
            srf_size_ecc_2.append(s_val)
            
            ecc_idx = (ecc >= 2.5) & (ecc < 3.5)
            p_val=np.nanmean(prf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            s_val=np.nanmean(srf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            #print("Ecc 3: "+str(p_val))
            prf_size_ecc_3.append(p_val)
            srf_size_ecc_3.append(s_val)
            
            ecc_idx = (ecc >= 3.5) & (ecc < 4.5)
            p_val=np.nanmean(prf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            s_val=np.nanmean(srf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            #print("Ecc 4: "+str(p_val))
            prf_size_ecc_4.append(p_val)
            srf_size_ecc_4.append(s_val)
            
            ecc_idx = (ecc >= 4.5) & (ecc < 5.5)
            p_val=np.nanmean(prf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            s_val=np.nanmean(srf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equivol == layer)])
            #print("Ecc 5: "+str(p_val))
            prf_size_ecc_5.append(p_val)
            srf_size_ecc_5.append(s_val)
        
        # save
        f = open(equivol_param_fn, 'wb')
        pickle.dump([prf_size_ecc_2,prf_size_ecc_3,prf_size_ecc_4,prf_size_ecc_5,srf_size_ecc_2,srf_size_ecc_3,srf_size_ecc_4,srf_size_ecc_5], f)
        f.close()
        ...
        df_equivol_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_2]*len(idx), index=df_equivol_per_ecc.index[idx])
        df_equivol_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_2]*len(idx), index=df_equivol_per_ecc.index[idx])

        idx=df_equivol_per_ecc.loc[(df_equivol_per_ecc['sub id'] == subject_list[sub_id]) & 
                           (df_equivol_per_ecc['hem'] == hem_list[hem_id]) & 
                           (df_equivol_per_ecc['ecc'] == 3)].index.tolist()
        df_equivol_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_3]*len(idx), index=df_equivol_per_ecc.index[idx])
        df_equivol_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_3]*len(idx), index=df_equivol_per_ecc.index[idx])

        idx=df_equivol_per_ecc.loc[(df_equivol_per_ecc['sub id'] == subject_list[sub_id]) & 
                           (df_equivol_per_ecc['hem'] == hem_list[hem_id]) & 
                           (df_equivol_per_ecc['ecc'] == 4)].index.tolist()
        df_equivol_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_4]*len(idx), index=df_equivol_per_ecc.index[idx])
        df_equivol_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_4]*len(idx), index=df_equivol_per_ecc.index[idx])

        idx=df_equivol_per_ecc.loc[(df_equivol_per_ecc['sub id'] == subject_list[sub_id]) & 
                           (df_equivol_per_ecc['hem'] == hem_list[hem_id]) & 
                           (df_equivol_per_ecc['ecc'] == 5)].index.tolist()
        df_equivol_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_5]*len(idx), index=df_equivol_per_ecc.index[idx])
        df_equivol_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_5]*len(idx), index=df_equivol_per_ecc.index[idx])



        ###########################################################################################
        ## Extract prf_size per ecc: equidistant
        prf_size_ecc_2 = []
        prf_size_ecc_3 = []
        prf_size_ecc_4 = []
        prf_size_ecc_5 = []
        ...
        for layer in range(2,np.size(np.unique(masked_lay_equidist))):
            ...
            #print('Layer '+str(layer))
            ecc_idx = (ecc >= 1.5) & (ecc < 2.5)
            p_val=np.nanmean(prf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equidist == layer)])
            s_val=np.nanmean(srf_size[(ecc_idx) & (masked_V1==1) & (total_rsq>rsq_thresh)  & (masked_lay_equidist == layer)])
            #print("Ecc 2: "+str(p_val))
            prf_size_ecc_2.append(p_val)
            srf_size_ecc_2.append(s_val)
        ...
        df_equidist_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_2]*len(idx), index=df_equidist_per_ecc.index[idx])
        df_equidist_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_2]*len(idx), index=df_equidist_per_ecc.index[idx])

        idx=df_equidist_per_ecc.loc[(df_equidist_per_ecc['sub id'] == subject_list[sub_id]) & 
                           (df_equidist_per_ecc['hem'] == hem_list[hem_id]) & 
                           (df_equidist_per_ecc['ecc'] == 3)].index.tolist()
        df_equidist_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_3]*len(idx), index=df_equidist_per_ecc.index[idx])
        df_equidist_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_3]*len(idx), index=df_equidist_per_ecc.index[idx])

        idx=df_equidist_per_ecc.loc[(df_equidist_per_ecc['sub id'] == subject_list[sub_id]) & 
                           (df_equidist_per_ecc['hem'] == hem_list[hem_id]) & 
                           (df_equidist_per_ecc['ecc'] == 4)].index.tolist()
        df_equidist_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_4]*len(idx), index=df_equidist_per_ecc.index[idx])
        df_equidist_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_4]*len(idx), index=df_equidist_per_ecc.index[idx])

        idx=df_equidist_per_ecc.loc[(df_equidist_per_ecc['sub id'] == subject_list[sub_id]) & 
                           (df_equidist_per_ecc['hem'] == hem_list[hem_id]) & 
                           (df_equidist_per_ecc['ecc'] == 5)].index.tolist()
        df_equidist_per_ecc.loc[idx, 'pRF size'] = pd.Series([prf_size_ecc_5]*len(idx), index=df_equidist_per_ecc.index[idx])
        df_equidist_per_ecc.loc[idx, 'sRF size'] = pd.Series([srf_size_ecc_5]*len(idx), index=df_equidist_per_ecc.index[idx])

        ###########################################################################################
        ## Extract sigma per depth: equivolumetric
        depth_idx=0
        for layer in range(2,np.size(np.unique(masked_lay_equivol))):
            prfxlayer = prf_size[(masked_V1==1) & (masked_lay_equivol == layer)]
            srfxlayer = srf_size[(masked_V1==1) & (masked_lay_equivol == layer)]
            ...
            df_equivol_per_depth.loc[idx, 'pRF size'] = pd.Series([prfxlayer]*len(idx), index=df_equivol_per_depth.index[idx])
            df_equivol_per_depth.loc[idx, 'sRF size'] = pd.Series([srfxlayer]*len(idx), index=df_equivol_per_depth.index[idx])
        ...
        depth_idx=0
        for layer in range(2,np.size(np.unique(masked_lay_equidist))):
            prfxlayer = prf_size[(masked_V1==1) & (masked_lay_equidist == layer)]
            srfxlayer = srf_size[(masked_V1==1) & (masked_lay_equidist == layer)]
            ...
            df_equidist_per_depth.loc[idx, 'pRF size'] = pd.Series([prfxlayer]*len(idx), index=df_equidist_per_depth.index[idx])
            df_equidist_per_depth.loc[idx, 'sRF size'] = pd.Series([srfxlayer]*len(idx), index=df_equidist_per_depth.index[idx])
